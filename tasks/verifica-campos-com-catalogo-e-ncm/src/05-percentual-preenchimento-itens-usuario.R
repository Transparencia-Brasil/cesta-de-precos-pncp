...
itens <- readRDS(PATH_ITENS)
usuarios <- readRDS(PATH_USUARIO)


# JOIN USUARIOS ----------------------------------------------------------------

usuarios <- usuarios %>%
  filter(str_detect(usuarioNome, "^[^0-9]+$")) %>%
  distinct(numeroControlePNCPCompra, usuarioNome)
...
contagem <- itens %>%
  count(coleta, name = "total")

itens %>%
  filter(is.na(usuarioNome)) %>%
  count(coleta, name = "total_sem_usuario") %>%
  left_join(contagem, .) %>%
  mutate(
    total_com_usuario = total - replace_na(total_sem_usuario, 0),
    perc = total_com_usuario / total
  )
