...
# seleciona somente Amoxicilina
catalogo_amoxicilina <- catalogo %>%
  filter(nome_pdm == "Amoxicilina")

# Desaninha a coluna "buscaItemCaracteristica"
catalogo_amoxicilina <- catalogo_amoxicilina %>% unnest(buscaItemCaracteristica)

# de-para para corrigir os códigos de características
dicionario_caracteristica <- catalogo_amoxicilina %>%
  distinct(nomeCaracteristica, codigoCaracteristica)

# Corrige características manualmente
catalogo_amoxicilina <- catalogo_amoxicilina %>%
  mutate(
...
caracteristicas_amoxicilina <- catalogo_amoxicilina %>%
  summarise(
...
catalogo_amoxicilina <- catalogo_amoxicilina %>%
  left_join(caracteristicas_amoxicilina, by = c("codigo_pdm", "codigoCaracteristica"))
