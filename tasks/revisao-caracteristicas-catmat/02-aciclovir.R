...
# seleciona somente Aciclovir
catalogo_aciclovir <- catalogo %>%
  filter(nome_pdm == "Aciclovir")

# Desaninha a coluna "buscaItemCaracteristica"
catalogo_aciclovir <- catalogo_aciclovir %>% unnest(buscaItemCaracteristica)

# de-para para corrigir os códigos de características
dicionario_caracteristica <- catalogo_aciclovir %>%
  distinct(nomeCaracteristica, codigoCaracteristica)

# Corrige características manualmente
catalogo_aciclovir <- catalogo_aciclovir %>%
  mutate(
...
caracteristicas_aciclovir <- catalogo_aciclovir %>%
  summarise(
...
catalogo_aciclovir <- catalogo_aciclovir %>%
  left_join(caracteristicas_aciclovir, by = c("codigo_pdm", "codigoCaracteristica"))
