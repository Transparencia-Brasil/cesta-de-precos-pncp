...
carrega_template_coleta <- function(alias, templates_dir = here::here("src/ETL/template/templates")) {
  ...
  if (!alias_normalizado %in% names(templates))  stop("Template desconhecido: ", alias)

  path_template <- file.path(templates_dir, templates[[alias_normalizado]])
  if (!file.exists(path_template)) stop("Arquivo de template nao encontrado: ", path_template)
  ...
  if (length(colunas_faltantes) > 0) stop("Template sem colunas obrigatorias: ", paste(colunas_faltantes, collapse = ", "))
